...
public final class ViewRootImpl implements ViewParent,
    ...
    private void deliverInputEvent(QueuedInputEvent q) {
		...
		// Recordroid Patch: view input event
		long startTimeMS = 0;
		if(q.mEvent instanceof KeyEvent) {
			startTimeMS = ((KeyEvent)q.mEvent).getDownTime();
		} else {
			startTimeMS = ((MotionEvent)q.mEvent).getDownTime();
		}
		...
		// Recordroid Patch: view input event
		try {
			IRecordroidService recordroid = IRecordroidService.Stub.asInterface(ServiceManager.getService("Recordroid"));
			recordroid.onViewInputEvent(startTimeMS);
		} catch(Exception e) {
			//Recordroid Service cannot found: ViewRootImpl.deliverInputEvent()
		}
    }
